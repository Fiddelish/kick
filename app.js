...
const KICKBOT_WEBSOCKET_URL = "wss://kickbot.live/ws";
const KICKBOT_STREAMER_ID = 173210;
...
const kickbotConnectionDot = document.querySelector("#kickbot-connection-dot");
const kickbotConnectionStatus = document.querySelector("#kickbot-connection-status");
...
let socket = null;
let kickbotStatusSocket = null;
let kickbotReconnectTimer = null;
let kickbotReloadTimer = null;
let activeKickbotUrl = "";
let lastKickbotReloadAt = 0;
...
function setKickbotConnection(state, text) {
  kickbotConnectionDot.className = `connection-dot ${state}`.trim();
  kickbotConnectionStatus.textContent = text;
}

function getKickbotAuthorization(url) {
  const pathname = new URL(url).pathname;
  const secret = pathname.split("/").filter(Boolean).at(-1) || "";
  return decodeURIComponent(secret);
}

function reloadKickbotFrame() {
  if (!activeKickbotUrl || Date.now() - lastKickbotReloadAt < 2000) return;
  lastKickbotReloadAt = Date.now();
  if (kickbotReloadTimer) window.clearTimeout(kickbotReloadTimer);
  kickbotFrame.src = "about:blank";
  kickbotReloadTimer = window.setTimeout(() => {
    kickbotFrame.src = activeKickbotUrl;
  }, 150);
}

function scheduleKickbotRecovery(url) {
  if (kickbotReconnectTimer) window.clearTimeout(kickbotReconnectTimer);
  if (document.hidden || !navigator.onLine || activeKickbotUrl !== url) return;
  kickbotReconnectTimer = window.setTimeout(() => {
    if (activeKickbotUrl !== url) return;
    reloadKickbotFrame();
    connectKickbotStatus(url);
  }, 5000);
}

function connectKickbotStatus(url) {
  activeKickbotUrl = url;
  if (kickbotReconnectTimer) window.clearTimeout(kickbotReconnectTimer);
  const previousSocket = kickbotStatusSocket;
  kickbotStatusSocket = null;
  if (previousSocket) previousSocket.close();

  setKickbotConnection("", "Kickbot ansluter");
  const authorization = getKickbotAuthorization(url);
  const statusSocket = new WebSocket(KICKBOT_WEBSOCKET_URL);
  kickbotStatusSocket = statusSocket;

  const connectionTimeout = window.setTimeout(() => {
    if (statusSocket.readyState === WebSocket.CONNECTING) statusSocket.close();
  }, 10000);
  ...
  statusSocket.addEventListener("open", () => {
    ...
    statusSocket.send(JSON.stringify({
      type: "subscribe",
      channel: `tipping_${KICKBOT_STREAMER_ID}`,
      authorization,
    }));
    setKickbotConnection("connected", "Kickbot online");
  });

  statusSocket.addEventListener("error", () => {
    if (kickbotStatusSocket === statusSocket) setKickbotConnection("error", "Kickbot fel");
  });

  statusSocket.addEventListener("close", () => {
    window.clearTimeout(connectionTimeout);
    if (kickbotStatusSocket !== statusSocket) return;
    kickbotStatusSocket = null;
    setKickbotConnection("error", "Kickbot offline");
    scheduleKickbotRecovery(url);
  });
}

function resumeKickbot() {
  if (!activeKickbotUrl || document.hidden || !navigator.onLine) return;
  reloadKickbotFrame();
  connectKickbotStatus(activeKickbotUrl);
}

document.addEventListener("visibilitychange", () => {
  if (document.visibilityState === "visible") resumeKickbot();
});
window.addEventListener("online", resumeKickbot);
window.addEventListener("pageshow", (event) => {
  if (event.persisted) resumeKickbot();
});
...
startForm.addEventListener("submit", (event) => {
  ...
  unlockAudio();
  activeKickbotUrl = kickbotUrl;
  kickbotFrame.src = kickbotUrl;
  connectKickbotStatus(kickbotUrl);
